...
| 6 | 2:20–2:45 | Terminal split: CPU run vs `python -m cudf.pandas` GPU run → the speedup number | "And acceleration earns its place. Our strategy-validation pipeline — the heavy cross-validation step — runs the *same* pandas code on an NVIDIA GPU with cuDF, zero code change. CPU took ~13 seconds; GPU does it in [X], a [Y]× speedup. Faster validation means a faster, better decision in your hands." |
...
## Recording notes
- Fill `[X]` / `[Y]` from the GPU benchmark (step 3) before recording.
